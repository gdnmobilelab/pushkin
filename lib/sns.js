...
export default {
    addTopicSubscription(topic, awsConfig) {
        return Promise.promisify(lambda.createEventSourceMapping, {context: lambda})({
            EventSourceArn: createArn('sns', process.env.SERVERLESS_REGION, AWS_ACCOUNT_ID, topic),
            FunctionName: createArn('lambda', process.env.SERVERLESS_REGION, AWS_ACCOUNT_ID, LISTENER_FUNC_PATH),
            Enabled: true,
