...
export default function(topicName, sns) {
    ...
    return redisClient.zcountAsync(namespacedTopic, "-inf", "+inf")
    ...
    .then((batchStartIndexes) => {
        ...
        return Promise.map(batchStartIndexes, (index) => {
            return redisClient.zrangeAsync([namespacedTopic, index, index, 'WITHSCORES']);
        })
        .map((arr) => {
        ...
        return redisClient.hmsetAsync(multiSetArgs)
        .then(() => {
            return Promise.map(ranges, (range, i) => {
