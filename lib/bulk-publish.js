...
export default {
    send: function(topic, [rangeStart, rangeEnd], message, broadcastIndex) {
        ...
        let {ttl, payload} = message.Message;

        if (!ttl || !payload) {
            throw new Error("Messages must now be sent as an object with ttl and payload keys.")
        }
        ...
        return withRedis((redisClient) => {
            ...
            .then((results) => {
                ...
                return PromiseTools.map(resultObjs, ({payload, score}) => {
                    return webPush.sendNotification(payload.endpoint, {
                        payload: payload,
                        ttl: TTL,
