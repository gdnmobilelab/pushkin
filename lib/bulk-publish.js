...
const removeNotRegisteredEndpoint = function (topic, endpoint, redisClient) {
    return removeSubscription(topic, {type: 'web', data: endpoint})
    .then(() => {
        return redisClient.lpush(namespaces.unsubscribedNotRegistered, JSON.stringify({topic, endpoint}));
    })
}

const removeNotRegisteredEndpoint = function (topic, endpoint) {
